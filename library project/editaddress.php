...
<body>
    				...
    				<div href="#" class="thumbnail">
      				<div class="row">
      					<div class="col-md-12 font">
      						If you want to change the address associated with your Libraria customer account, you may do so below. Be sure to click the Save Changes button when you are done.
      					</div>
      				</div>
      				<br><br>
      				<div class="row">
      					<div class="col-md-3"><b>New Address</b></div>
      				</div>
      				<br>
      				<div class="row">
      					<div class="col-md-5">
      						<input type="text" class="form-control" id="exampleInputEmail1" placeholder="Address" name="newusername" required>
      					</div>
      				</div>
      				</div>
  ...
  if(isset($_POST['submitsavechanges'])){
     ...
     if(mysqli_num_rows($query_run)==0){
      $query="update user_info set address='$newusername' where username='$sai'";
     $query_run = mysqli_query($con,$query);
     if ($query_run) {   
         echo '<script type="text/javascript"> alert("Your address is changed") </script>';
     }
     else{
