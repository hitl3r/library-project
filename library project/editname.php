...
<body>
    				...
    				<div href="#" class="thumbnail">
      				<div class="row">
      					<div class="col-md-12 font">
      						If you want to change the name associated with your Libraria customer account, you may do so below. Be sure to click the Save Changes button when you are done.
      					</div>
      				</div>
      				<br><br>
      				<div class="row">
      					<div class="col-md-3"><b>New Name</b></div>
      				</div>
      				<br>
      				<div class="row">
      					<div class="col-md-5">
  ...
  if(isset($_POST['submitsavechanges'])){
     ...
     if(mysqli_num_rows($query_run)==0){
      $query="update user_info set username='$newusername' where username='$sai'";
     $query_run = mysqli_query($con,$query);
     if ($query_run) {   
         $qwe=$_SESSION['username'];
         $query="UPDATE `bookings` SET `username`='$newusername' WHERE username='$qwe'  ";
         $query_run=mysqli_query($con,$query);
         if($query_run){
          $_SESSION['username']=$newusername;
         $_SESSION['usrname']=$newusername;

         echo '<script type="text/javascript"> alert("Your username is changed") </script>';

         }
         else{
          echo '<script type="text/javascript"> alert("my bookings error") </script>';
